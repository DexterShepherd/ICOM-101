...
- Software
  __we will install these together as the course progresses__
...
### Schedule
__Schedule is subject to change at any time__
...
- Design and code a document with html on codepen
  - Resume/CV would be a good starting point, but go as far as you like
  - Make sure to use different heading levels with `h1` through `h6` tags
  - use the `p` tag to format text into paragraphs
  - divide your content into sections with `div`, `section`, and `article` tags
...
- Add some CSS to style your html
  - Use the CSS Box model to add `padding`, `margin`, or `border` to an element
  - Change the `color` or `background-color` of an element
  - Change the font of an element from the default
    - Extra credit : Load a web font from google fonts
...
__Homework__
The things we learned in class this week are a little less linear, so for this assignment feel free to break away from the `styled text document` idea we have been working with. Try making a landing page, a looping animation, or just experiment and play with every we've learned so far. Don't feel pressured to make the final product a __website__.
...
- Leaving Codepen!
  ...
  - Using devtools
- Going over content for the quiz
